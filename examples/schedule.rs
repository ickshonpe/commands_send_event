...
fn writer(mut event: EventWriter<MyEvent>, mut commands: Commands) {
    ...
    commands.send_event(MyEvent("Sent went Commands::send_event".to_string()));
}
...
pub fn main() {
    App::new()
        .add_event::<MyEvent>()
        .add_system((|| println!("* CoreStage::Update *")).label("print stage"))
        .add_system(reader.after("print stage").before("writer"))
        .add_system(writer.label("writer"))
        .add_system(reader.after("writer"))
        .add_system_to_stage(
            CoreStage::PostUpdate,
            (|| println!("* CoreStage::PostUpdate *")).label("print stage"),
        )
        .add_system_to_stage(CoreStage::PostUpdate, reader.after("print stage"))
        .run();
}
